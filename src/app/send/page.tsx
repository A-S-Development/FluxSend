...
const SendHomePage: React.FC = () => {
  ...
  const preview = () => {
    if (file.type === 'image/jpeg' || file.type === 'image/png') {
      return <Image src={previewUrl} width={150} height={150} alt="picture" />;
    } else if (file.type === 'application/pdf') {
      return <PictureAsPdfIcon width={150} height={150} />;
    } else if (file.type === 'audio/mpeg' || file.type === 'audio/wav') {
      return <AudioFileIcon width={150} height={150} />;
    }
  };
        ...
        <Button
          ...
          onClick={() => {
            if (fileInput.current) {
              fileInput.current?.click(generateSessionLink);
            }
          }}
        >
          Stage File
